...
def main():
    ...
    run = True
    score = 0
    apple_add_increment = 2000
    apple_count = 0
    apples = []
    hit = False

    apple_enemy = pygame.Rect(apples.x, apples.y, APPLE_WIDTH, APPLE_HEIGHT)
    basket_player = pygame.Rect(400, 550, BASKET_WIDTH, BASKET_HEIGHT)

    while run:
        apple_count += clock.tick(FPS)
        ...
        for apple in apples[:]:
            ...
            if apple.y > HEIGHT:
                apples.remove(apple)
            elif apple.y >= basket_player.y and apple.colliderect(basket_player):
                apples.remove(apple)
                hit = True
                break
